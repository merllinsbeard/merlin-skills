...
`brainstorming -> spec-kit -> create-goal -> /goal -> tdd -> gstack-qa -> playwright proof -> gstack-ship`
...
Most upstream skills are copied as upstream snapshots. The adapted skill sources are `skills/create-goal`, `skills/qa`, and `skills/ship`: goal creation is rewritten for spec-kit, while the retained gstack QA/ship flows are rewritten as standalone Merlin gates.
...
5. Use implementation skills only when routed. Gstack is deliberately minimal in this stack: QA and ship only.
...
| UI plan | `brainstorming -> spec-kit -> playwright-cli` |
